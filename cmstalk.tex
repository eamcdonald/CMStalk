...
%     Since $H$ is lower bounded and has no essential spectrum, we have $N(t,H) < \infty$ for all $t \in \Rl$.
\end{frame}
...
\begin{frame}
% One important question regarding DOS is its existence. DOS does not always exists, but it is well known that it exists for \\
% 
% 1) for Schr\"odinger operators with periodic potentials;\\
...
% Existence of DOS is a subtle question. We will simply assume that it exists. 
...
%     $$
%         \|T\|_{1,\infty} := \sup_{k\geq 0}\;(1+k)\mu(k,T) < \infty.
%     $$
...
%     If $0\leq T \in \mathcal{L}_{1,\infty}$, then
%     \begin{equation*}
%         \sum_{k=0}^N \mu(k,T) = \mathcal{O}(\log(N)),\quad N\to\infty.
%     \end{equation*}
%     \pause   
...
% {\Huge{}(switch to document camera)}
